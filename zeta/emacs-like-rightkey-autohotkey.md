...
しかし、実際にコーディングをしていたりすると漠然と以下の2つの悩みがあった。
...
後々調べた結果、その操作は「Emacsキーバインド」と呼ばれるもので、ホームポジションから手を離すことなく操作することを念頭に作られたエディターがVimとEmacsだった。
...
qiita: "(![Google日本語入力の設定](https://qiita-image-store.s3.ap-northeast-1.amazonaws.com/0/432981/0733a0d0-3fc6-c953-bf09-6cb04ab0c91a.png))"
</macro>

### AutoHotKeyスクリプト

今回AutoHotKeyのスクリプトは自作した。
先駆者は結構いるのだが、2022年にAutoHotKeyのバージョンがv2に上がっており、せっかくだし自分で1から作成し直すことにした。
...
WindowsとMacで度々間違うことも減るだろう。（コピペとかはいまだによく間違うけど・・・）
